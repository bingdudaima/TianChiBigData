...
Data = pd.read_csv('../DataSet/subData.csv')
...
def get_train(train_user,end_time):
    #取出label day 前一天的记录作为打标记录,timedelta表示时间差
...
def get_label_testset(train_user,LabelDay):
    #测试集选为上一天所有的交互数据
    data_test = train_user[(train_user['daystime'] == LabelDay)]
...
#    sys.exit()
    ...
    new_train_set_0 = train_set_0.sample(len(train_set_1)*90)#sample里的参数为采样数目，这里注意train_set_0的采样数为train_set_1数目的90倍
...
#    predict1.to_csv("../result/10_30_2/650_1B80minchildweight1.8.csv", index=False)
    ...
    # 保存到文件
    predict2.to_csv("../DataSet/result.csv", index=False)
